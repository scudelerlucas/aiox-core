...
## Formato de Resposta ao Operador (Lucas) — OBRIGATÓRIO (PERMANENTE)

> Adicionado em 2026-07-13, por pedido do operador. Vale em TODAS as respostas.

Ao FINAL de CADA resposta, incluir SEMPRE, em linguagem simples (sem jargão técnico):

1. **📋 Resumo simples** — o que foi feito/descoberto, em 2-5 frases claras.
2. **🔀 Decisões que você precisa tomar** — para cada decisão real do operador,
   listar as opções com: **Recomendo** (qual + por quê, 1 frase) e **Riscos de cada
   opção**. Se não houver, escrever "Nenhuma decisão pendente."
3. **🪜 Passo a passo (quando houver)** — instruções numeradas em linguagem simples,
   com os **links diretos** de cada passo.

Regras: nunca pedir dados técnicos ao Lucas (DevTools/Network/logs) — diagnosticar
sozinho; sempre incluir o link respectivo ao citar uma tela/serviço; escrever para
leitor não-técnico (curto, concreto, sem prefácio).

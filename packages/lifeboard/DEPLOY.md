...
| — | `tests/fila_prompts.test.sql` | A guarda comportamental. **Quantos blocos, e quais, está em `tests/BLOCOS.txt`** — o manifesto nominal, que é também o que o runner exige por nome (CRÍTICO 1 da rodada 13: este número dizia 68 quando já eram 74, e era o próprio arquivo que contava a si mesmo). T60–T62 são da rodada pós-merge (exigem a `0025`); T63–T74 são das rodadas 12 e 13 e exigem a `0027` e a `0028`; T75–T78 são da rodada 13; T79–T85 são da rodada 14 e exigem a `0029`; T86–T91 são da rodada 15 e exigem a `0030`; T92 (a célula mostra o lançamento vigente), T93 (o ajuste confere a sessão proposta) e T94 (trocar de sessão não apaga o que a antiga publicou), T95 (o histórico do card tem piso zero) e T96 (a escolha automática desvia da conta no limite de sessões em voo) exigem a `0027`, a `0029` e a `0030` como ficaram no PR #42. |

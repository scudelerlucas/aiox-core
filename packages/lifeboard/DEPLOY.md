...
| **D7** | **`pega` não é beco sem saída** | `fila_prompts_cancelar(p_secret, p_id)` aceita `na_fila` **e** `pega`; o item para de reservar na hora e `fila_prompts_heartbeat_interno` passa a devolver `{ok:false, motivo:'cancelado'}` — o worker interrompe a sessão filha (`interrupt_session`). **A vaga de sessão em voo, não:** ela continua ocupada até o worker daquele item ouvir o `cancelado` no heartbeat ou fechar o item com o número medido (ou a janela de 45 min vencer) — sem isso, cancelar com a conta no limite abria uma quinta sessão com a quarta ainda rodando (P2 do Codex no PR #42, 10ª rodada; coluna `parada_pendente_desde`, 0030; bloco T100). Na tela, o botão cancelar aparece nos dois estados. |
